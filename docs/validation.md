...
| `CALCULATED_VALUE_MISMATCH`   | In Complete mode, a submitted calculated value differs from the computed one (integers and doubles compare numerically, tolerance 1e-6 — the same equality the `eq` operator uses). Applies to repeater children per row, symmetric with scalar fields |
| `VALIDATION_ERRORS_TRUNCATED` | More than 100 errors were collected; the last entry carries the total (S-7's rule applied to response validation)                                                                                                                                      |
