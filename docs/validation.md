...
| `CALCULATED_VALUE_MISMATCH` | In Complete mode, a submitted calculated value differs from the computed one (doubles compare within 1e-6)    |
| `REPEATER_MIN_ITEMS`        | In Complete mode, fewer rows than `minItems`                                                                  |
| `REPEATER_MAX_ITEMS`        | In Complete mode, more rows than `maxItems`                                                                   |
| `CALCULATED_VALUE_INVALID`  | Declared, but unreachable: a non-finite calculation is turned into `null` before this check                   |

Emptiness for `required` purposes: an absent key, `null`, `""`, `[]` and `{}` are
empty. `0` and `false` are **not** empty.
