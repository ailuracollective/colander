...
use super::super::check::{EvalBudget, Stop, check_into, evaluate};
...
pub(crate) fn check_combinators(
    ...
    if let Some(all_of) = json::get_array(schema, "allOf") {
        for sub_schema in all_of {
            if let Some(stop) = check_into(sub_schema, instance, root, budget, errors) {
                return Some(stop);
            }
        }
    }
    ...
    if let Some(any_of) = json::get_array(schema, "anyOf") {
        ...
        for sub_schema in any_of {
            let (branch_errors, stop) = evaluate(sub_schema, instance, root, budget);
    ...
    if let Some(negated) = json::get(schema, "not") {
        let (negated_errors, stop) = evaluate(negated, instance, root, budget);
        if let Some(stop) = stop {
            errors.push_stop(stop);
            return Some(stop);
        }
    ...
    if let Some(condition) = json::get(schema, "if") {
        let (condition_errors, stop) = evaluate(condition, instance, root, budget);
        if let Some(stop) = stop {
            errors.push_stop(stop);
            return Some(stop);
        }
        if condition_errors.is_empty() {
            if let Some(then_schema) = json::get(schema, "then")
                && let Some(stop) = check_into(then_schema, instance, root, budget, errors)
            {
                return Some(stop);
            }
        } else if let Some(else_schema) = json::get(schema, "else")
            && let Some(stop) = check_into(else_schema, instance, root, budget, errors)
        {
            return Some(stop);
        }
    }
